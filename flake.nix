{
  ...
  outputs =
    ...
    let
      ...
      ktlspCargoToml = builtins.fromTOML (builtins.readFile ./crates/ktlsp/Cargo.toml);
      ktcheckCargoToml = builtins.fromTOML (builtins.readFile ./crates/ktcheck/Cargo.toml);
      cargoOutputHashes = {
        "tree-sitter-kotlin-ng-1.1.0" = "sha256-+e1Tgd/VRXkP4FKbt+44xUVcGeqk81nSuzqgqhH8mBU=";
      };
      ...
      mkSidecar =
        ...
        pkgs.stdenv.mkDerivation (finalAttrs: {
          pname = "ktlsp-sidecar";
          version = ktlspCargoToml.package.version;
      ...
      mkKtlsp =
        ...
        pkgs.rustPlatform.buildRustPackage {
          pname = ktlspCargoToml.package.name;
          version = ktlspCargoToml.package.version;
          ...
          meta = {
            description = ktlspCargoToml.package.description;
      ...
      mkKtcheck =
        pkgs:
        pkgs.rustPlatform.buildRustPackage {
          pname = ktcheckCargoToml.package.name;
          version = ktcheckCargoToml.package.version;
          ...
          meta = {
            description = ktcheckCargoToml.package.description;
            homepage = "https://github.com/pepegar/ktlsp";
            license = lib.licenses.mit;
            mainProgram = "ktcheck";
            platforms = systems;
    ...
    in
    {
      ...
      #   nixpkgs.overlays = [ ktlsp.overlays.default ];  # then pkgs.ktlsp and pkgs.ktcheck exist
      overlays.default = _final: prev: {
        ktlsp = mkKtlsp prev;
        ktcheck = mkKtcheck prev;
      };

      # `nix build`, and for consumers: `ktlsp.packages.${system}.default`.
      packages = forAllSystems (pkgs: rec {
        ktlsp = mkKtlsp pkgs;
        ktcheck = mkKtcheck pkgs;
        sidecar = mkSidecar pkgs;
        default = ktlsp;
      });
      ...
      apps = forAllSystems (
        ...
        in
        {
          ktcheck = {
            type = "app";
            program = lib.getExe (mkKtcheck pkgs);
          };
          ktlsp = app;
          default = app;
        }
