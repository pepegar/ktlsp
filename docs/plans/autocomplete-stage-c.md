...
- src/complete.rs (ranking, snippets, detail, the pure `shape` core; `ScopeCompletion` extension; `ImportAnchor`)
- src/workspace.rs (`complete` shaping pass; `import_layout`; candidate field stamping + visibility/`import_path`)
- src/lsp.rs (snippet-capability thread; `ShapedItem` → `CompletionItem` mapping incl. `additionalTextEdits`; `CompletionResponse::List`)
- src/symbol.rs (add `arity` to `IndexedSymbol`)
- src/indexer.rs (populate `arity`)
- src/deps.rs (`SYMCACHE_VERSION` bump)
- tests/completion.rs (ordered `ShapedItem` harness + ranking/snippet/auto-import/silent-omission fixtures)
