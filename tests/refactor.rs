...
fn expression_body_can_convert_to_block_body() {
    ...
    let actions = ws.code_actions(&key, src.find("42").unwrap(), src.find("42").unwrap(), src.find("42").unwrap());
    let action = actions
        .iter()
...
fn simple_block_body_can_convert_to_expression_body() {
    ...
    let actions = ws.code_actions(&key, src.find("return").unwrap(), src.find("return").unwrap(), src.find("return").unwrap());
    let action = actions
        .iter()
...
fn multi_statement_block_body_has_no_expression_conversion() {
    ...
    assert!(
        !actions.iter().any(|action| action.title == "Convert block body to expression body"),
