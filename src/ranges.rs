...
/// Foldable regions derived from stable tree-sitter nodes plus top-level import groups.
...
/// Selection range parent chain for `offset`, from the smallest named node outward.
...
fn node_at<'t>(root: Node<'t>, text: &str, offset: usize) -> Option<Node<'t>> {
    ...
    let prefer_prev = prev.is_some_and(|start| {
        text[start..].chars().next().is_some_and(is_ident_part)
    });
