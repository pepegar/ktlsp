...
pub fn import_lines(tree: &Tree, src: &str) -> Vec<ImportLine> {
    ...
    for child in root.named_children(&mut cursor) {
        ...
        out.push(ImportLine {
            ...
            text: node_text(child, src).trim().to_string(),
        });
    }
    out
}
...
pub fn organize_imports_edit(file: &str, tree: &Tree, src: &str) -> Option<TextEdit> {
    let imports = import_lines(tree, src);
    if imports.is_empty() {
        return None;
    }
    let first = imports.first()?;
    let last = imports.last()?;
    if has_named_non_import_between(tree.root_node(), first.start_byte, last.end_byte) {
        return None;
    }
    ...
    let mut lines = imports.iter().map(|imp| imp.text.clone()).collect::<Vec<_>>();
    lines.sort();
    lines.dedup();
    let new_text = format!("{}\n", lines.join("\n"));
...
fn has_named_non_import_between(root: Node<'_>, start_byte: usize, end_byte: usize) -> bool {
    let mut cursor = root.walk();
    for child in root.named_children(&mut cursor) {
