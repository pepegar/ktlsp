...
use crate::imports::{self, ImportLine};
...
pub fn unused_import_actions(
    ...
    let imports = imports::import_lines(tree, text);
    let mut out = Vec::new();
    let mut all_edits = Vec::new();

    for diagnostic in diagnostics.iter().filter(|d| d.code == Some(DiagnosticCode::UnusedImport)) {
        let Some(import) = imports
            .iter()
            .find(|import| import.start_byte == diagnostic.start_byte && import.end_byte == diagnostic.end_byte)
        else {
            continue;
        };
        let edit = imports::remove_import_edit(file, text, import);
        all_edits.push(edit.clone());
        if ranges_intersect(diagnostic.start_byte, diagnostic.end_byte, range_start, range_end) {
            out.push(Action {
...
pub fn organize_imports_action(file: &str, text: &str, tree: &Tree) -> Option<Action> {
    imports::organize_imports_edit(file, tree, text).map(|edit| Action {
...
pub fn add_import_action(file: &str, text: &str, tree: &Tree, name: &str, fqn: &str) -> Option<Action> {
    let (sorted_imports, anchor) = imports::import_layout(tree, text)?;
