...
/// Implementations of a type or member declaration.
///
/// Types map to their direct subtypes. Functions and properties map to matching declarations on
/// those direct subtypes, which is the member-level relationship exposed by
/// `textDocument/implementation` in editors.
pub fn implementations(index: &Index, target: &Entry) -> Vec<Def> {
    if target.sym.kind.is_type_like() {
        return type_implementations(index, &item_from_entry(target));
    }
    if !matches!(target.sym.kind, SymbolKind::Function | SymbolKind::Property) {
        return Vec::new();
    }
    let Some(owner_path) = target.sym.container.as_deref() else {
        return Vec::new();
    };

    let owners: Vec<Entry> = index
        .all_entries()
        .into_iter()
        .filter(|entry| entry.sym.kind.is_type_like())
        .filter(|entry| entry.sym.package == target.sym.package)
        .filter(|entry| type_container_path(entry) == owner_path)
        .collect();

    let mut out = Vec::new();
    for owner in owners {
        for subtype in direct_subtype_entries(index, &owner) {
            let subtype_path = type_container_path(&subtype);
            out.extend(
                index
                    .members_of(&subtype_path)
                    .iter()
                    .filter(|member| member.sym.package == subtype.sym.package)
                    .filter(|member| member.sym.container.as_deref() == Some(&subtype_path))
                    .filter(|member| member.sym.name == target.sym.name)
...
fn direct_subtype_entries(index: &Index, target: &Entry) -> Vec<Entry> {
    index
        .all_entries()
        .into_iter()
        .filter(|entry| entry.sym.kind.is_type_like())
        .filter(|entry| {
            entry.sym.name != target.sym.name || entry.sym.package != target.sym.package
        })
        .filter(|entry| {
            entry
                .sym
                .supertypes
                .iter()
                .any(|supertype| supertype == &target.sym.name)
        })
        .collect()
}

fn type_container_path(entry: &Entry) -> String {
    match entry.sym.container.as_deref() {
        Some(container) => format!("{container}.{}", entry.sym.name),
        None => entry.sym.name.clone(),
    }
}

fn member_signatures_match(target: &Entry, candidate: &Entry) -> bool {
    if target.sym.kind != SymbolKind::Function {
        return true;
    }
    if target.sym.arity != candidate.sym.arity
        || target.sym.params.len() != candidate.sym.params.len()
    {
        return false;
    }
    target
        ...
        .params
        .iter()
        .zip(&candidate.sym.params)
        .all(|(target, candidate)| type_ref_shapes_match(target, candidate))
}

fn type_ref_shapes_match(target: &TypeRef, candidate: &TypeRef) -> bool {
    target.name == candidate.name
        && target.nullable == candidate.nullable
        && target.args.len() == candidate.args.len()
        && target
            .args
            .iter()
            .zip(&candidate.args)
    ...
    index
        ...
        .filter(|entry| {
            entry
                ...
                .supertypes
                .iter()
                .any(|supertype| supertype == &item.name)
        })
    ...
    F: FnMut(&str) -> Option<(String, Tree)>,
{
    ...
    for r in refs {
        ...
        let Some(caller) = enclosing_callable_item(index, &r.file, &tree, &text, r.start_byte)
        else {
            continue;
        };
        grouped
...
pub fn outgoing_calls(
    ...
    let Some(decl) = declaration_node_for_range(tree, callable.start_byte, callable.end_byte)
    else {
        return Vec::new();
    };
    let mut grouped: BTreeMap<(String, usize, usize), (HierarchyItem, Vec<Def>)> = BTreeMap::new();
    visit_identifiers(decl, &mut |ident| {
        ...
        for def in resolve::goto(index, file, text, tree, ident.start_byte()) {
            let Some(entry) = entry_for_name_range(index, &def.file, def.start_byte, def.end_byte)
            else {
                continue;
            };
