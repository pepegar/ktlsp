...
fn organize_imports_declines_when_comments_are_interleaved() {
    ...
    assert!(
        !actions
            .iter()
            .any(|action| action.title == "Organize imports"),
...
fn add_import_action_is_absent_for_ambiguous_symbols() {
    let mut ws = Workspace::new();
    ws.open(
        "mem:///One.kt".to_string(),
        "package one\nclass HelperXyz\n".to_string(),
    );
    ws.open(
        "mem:///Two.kt".to_string(),
        "package two\nclass HelperXyz\n".to_string(),
    );
    let key = "mem:///Main.kt".to_string();
    let src = "package app\nfun main() { HelperXyz() }\n";
    ...
    assert!(
        !actions
            .iter()
            .any(|action| action.title == "Import `HelperXyz`"),
        "ambiguous add-import should be absent: {actions:?}"
    );
}

#[test]
fn java_remove_unused_import_action_deletes_the_import_line() {
    let mut ws = Workspace::new();
    ...
    let action = actions
        .iter()
        .find(|action| action.title == "Remove unused import `List`")
        .unwrap_or_else(|| panic!("missing Java remove action: {actions:?}"));
    let edited = apply_to_text(&key, src, &action.edits).unwrap();
...
#[test]
fn java_remove_all_unused_imports_action_deletes_every_unused_import() {
    let mut ws = Workspace::new();
    let key = "mem:///Main.java".to_string();
    ...
    let action = actions
        .iter()
        .find(|action| action.title == "Remove all unused imports")
        .unwrap_or_else(|| panic!("missing Java remove-all action: {actions:?}"));
    let edited = apply_to_text(&key, src, &action.edits).unwrap();
...
#[test]
fn java_organize_imports_action_sorts_import_block() {
    let mut ws = Workspace::new();
    let key = "mem:///Main.java".to_string();
    let src = "package app;\nimport z.Z;\nimport a.A;\nclass Main {}\n";
    ws.open(key.clone(), src.to_string());
    ...
    let action = actions
        .iter()
        .find(|action| action.title == "Organize imports")
        .unwrap_or_else(|| panic!("missing Java organize action: {actions:?}"));
    let edited = apply_to_text(&key, src, &action.edits).unwrap();
...
#[test]
fn java_add_import_action_inserts_unambiguous_indexed_symbol() {
    let mut ws = Workspace::new();
    ws.open(
        "mem:///Helper.java".to_string(),
        "package lib;\npublic class HelperXyz {}\n".to_string(),
    );
    let key = "mem:///Main.java".to_string();
    let src = "package app;\nclass Main { HelperXyz helper; }\n";
    ws.open(key.clone(), src.to_string());
    let offset = src.find("HelperXyz").unwrap();
    ...
    let action = actions
        .iter()
        .find(|action| action.title == "Import `HelperXyz`")
        .unwrap_or_else(|| panic!("missing Java add-import action: {actions:?}"));
    let edited = apply_to_text(&key, src, &action.edits).unwrap();
