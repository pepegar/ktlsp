...
fn check_none(input: &str) {
    ...
    assert!(
        got.is_none(),
        "expected None (silent omission), got {got:?}\nfixture:\n{input}"
    );
}
...
fn shadowing_one_label() {
    ...
    .unwrap();
    assert_eq!(
        got.iter().filter(|l| l.as_str() == "x").count(),
        1,
        "got {got:?}"
    );
}
...
fn cross_file_other_package_offered_with_auto_import() {
    ...
    let imp = widget
        .auto_import
        .as_ref()
        .expect("other-package symbol carries an auto-import");
...
fn partial_member_prefix_filters() {
    ...
    assert!(
        !got.contains("close"),
        "prefix `op` must exclude `close`: {got:?}"
    );
    assert!(
        !got.contains("size"),
        "prefix `op` must exclude `size`: {got:?}"
    );
}
...
fn ranking_shorter_before_longer_from_fixture() {
    ...
    assert!(
        ...
        "greet present: {labels:?}"
    );
    assert!(
        labels.contains(&"greeting".to_string()),
        "greeting present: {labels:?}"
    );
    assert!(
        !labels.contains(&"abgreet".to_string()),
        "abgreet must be filtered out: {labels:?}"
    );
    let gi = labels.iter().position(|l| l == "greet").unwrap();
    let gti = labels.iter().position(|l| l == "greeting").unwrap();
    assert!(
        gi < gti,
        "shorter `greet` ranks before `greeting`: {labels:?}"
    );
}
...
fn auto_import_for_type_from_another_package() {
    ...
    let imp = helper
        .auto_import
        .as_ref()
        .expect("auto_import present for unimported type");
...
fn no_auto_import_for_same_package_symbol() {
    ...
    let helper = item_with(&s, "Helper").expect("Helper present");
    assert_eq!(
        helper.auto_import, None,
        "same-package symbol must not auto-import"
    );
}
...
fn no_auto_import_for_already_imported_symbol() {
    ...
    assert_eq!(
        helper.auto_import, None,
        "already-imported symbol must not auto-import"
    );
}
...
fn extension_auto_import_by_own_fqn() {
    ...
    let imp = second
        .auto_import
        .as_ref()
        .expect("extension auto_import present");
    assert_eq!(
        imp.text, "import ext.second",
        "extension imports by its OWN fqn"
    );
...
fn extension_already_imported_no_auto_import() {
    ...
    let second = item_with(&s, "second").expect("extension `second` present");
    assert_eq!(
        second.auto_import, None,
        "wildcard-imported extension must not auto-import"
    );
}
...
#[test]
fn bare_member_in_apply_is_whole_receiver() {
    let input = "//- lib.kt\npackage app\nclass Foo {\n    fun bar() {}\n}\nfun makeFoo(): Foo = Foo()\n\
                 //- Main.kt\npackage app\nfun f() {\n    makeFoo().apply {\n        ba/*^*/\n    }\n}\n";
    check_contains(input, &["bar"]);
}
...
#[test]
fn bare_member_in_custom_receiver_lambda_is_visible() {
    let input =
        "//- lib.kt\npackage app\nclass Span {\n    fun setTag(key: String, value: String) {}\n}\n\
                 fun span(name: String = \"x\", block: Span.() -> Unit) {}\n\
                 //- Main.kt\npackage app\nfun f() {\n    span {\n        setT/*^*/\n    }\n}\n";
    check_contains(input, &["setTag"]);
}

#[test]
fn bare_outer_receiver_member_from_function_type_alias_is_visible() {
    let input = "//- Routing.kt\npackage routing\nclass RoutingCall\nclass RoutingContext(val call: RoutingCall)\nclass ApplicationCall\ntypealias RoutingHandler = suspend RoutingContext.() -> Unit\nfun post(body: RoutingHandler) {}\nfun RoutingContext.ok(body: suspend ApplicationCall.() -> Unit) {}\n\
                 //- Main.kt\npackage app\nimport routing.post\nimport routing.ok\nfun setup() {\n    post {\n        ok {\n            ca/*^*/\n        }\n    }\n}\n";
    check_contains(input, &["call"]);
}

#[test]
fn extension_function_receiver_completes_through_nested_overloaded_receiver_lambdas() {
    let input = "class Route\nclass RoutingCall\nclass RoutingContext(val call: RoutingCall)\nclass ApplicationCall\n\
                 typealias RoutingHandler = suspend RoutingContext.() -> Unit\n\
...
#[test]
fn ordinary_type_alias_does_not_contribute_receiver_members() {
    let input = "//- Lib.kt\npackage lib\nclass Receiver { val call: Int = 1 }\ntypealias Name = String\nfun post(body: Name) {}\n\
                 //- Main.kt\npackage app\nimport lib.post\nfun setup() { post { ca/*^*/ } }\n";
    check_none(input);
}
