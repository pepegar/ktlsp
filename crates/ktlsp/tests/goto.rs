...
fn parse_fixture(input: &str) -> Fixture {
    ...
    for line in input.lines() {
        if let Some(rest) = line.strip_prefix("//- ") {
            raw_files.push((
                name.take().unwrap_or_else(|| "Main.kt".into()),
    ...
    for (key, raw) in raw_files {
        assert!(
            ...
            "duplicate fixture file key: {key}"
        );
...
fn check(input: &str) {
    ...
    for (key, off) in &fx.defs {
        ...
        let id = identifier_at(&tree, *off).unwrap_or_else(|| {
            panic!(
                "/*def*/ marker in {key} at {off} is not on an identifier: {}",
...
#[test]
fn constructor_property_resolves_when_class_header_paren_starts_the_next_line() {
    check(
        "//- Service.kt\n\
         class Service {\n\
             fun execute() {}\n\
         ...
         class InvitationsApi\n\
             (\n\
             private val /*def*/createInvitationUseCase: Service,\n\
         ) {\n\
             fun setup() {\n\
...
#[test]
fn constructor_property_declaration_resolves_to_itself_with_split_class_header() {
    check(
        "class Service\n\
         class InvitationsApi\n\
             (\n\
...
#[test]
fn implicit_receiver_member_picks_overload_via_apply_lambda() {
    check(
        "class A {\n    fun /*def*/run2() {}\n}\nclass B {\n    fun run2() {}\n}\nfun makeA(): A = A()\nfun caller() {\n    makeA().apply {\n        /*^*/run2()\n    }\n}\n",
    );
}
...
#[test]
fn implicit_receiver_member_resolves_custom_receiver_lambda() {
    check(
        "class Span {\n    fun /*def*/setTag(key: String, value: String) {}\n}\nfun span(name: String = \"x\", block: Span.() -> Unit) { }\nfun caller() {\n    span {\n        /*^*/setTag(\"a\", \"b\")\n    }\n}\n",
    );
}

#[test]
fn implicit_receiver_member_resolves_through_function_type_alias() {
    check(
        "//- Routing.kt\npackage routing\nclass RoutingCall\nclass RoutingContext(val /*def*/call: RoutingCall)\nclass ApplicationCall\ntypealias RoutingHandler = suspend RoutingContext.() -> Unit\nfun post(body: RoutingHandler) {}\nfun RoutingContext.ok(body: suspend ApplicationCall.() -> Unit) {}\n\
         //- Main.kt\npackage app\nimport routing.post\nimport routing.ok\nfun setup() {\n    post {\n        ok {\n            /*^*/call\n        }\n    }\n}\n",
    );
}

#[test]
fn extension_function_receiver_feeds_nested_overloaded_receiver_lambdas() {
    check(
        "class Route\nclass RoutingCall\nclass RoutingContext(val /*def*/call: RoutingCall)\nclass ApplicationCall\n\
         typealias RoutingHandler = suspend RoutingContext.() -> Unit\n\
         fun <R> Route.post(body: suspend RoutingContext.(R) -> Unit) {}\n\
         fun Route.post(body: RoutingHandler) {}\n\
         fun Route.route(path: String, body: Route.() -> Unit) {}\n\
         class VersionedRouteBuilder { fun allVersionsRoutes(body: Route.() -> Unit) {} }\n\
...
#[test]
fn inner_receiver_still_shadows_alias_backed_outer_receiver() {
    check(
        "class RoutingCall\nclass RoutingContext(\n    val call: RoutingCall,\n)\nclass ApplicationCall(\n    val /*def*/call: RoutingCall,\n)\ntypealias RoutingHandler = suspend RoutingContext.() -> Unit\nfun post(body: RoutingHandler) {}\nfun RoutingContext.ok(body: suspend ApplicationCall.() -> Unit) {}\nfun setup() { post { ok { /*^*/call } } }\n",
    );
}

#[test]
fn conflicting_receiver_function_aliases_stay_unresolved() {
    check(
        "//- A.kt\npackage routing\nclass A(val call: Int)\ntypealias Handler = suspend A.() -> Unit\n\
         //- B.kt\npackage routing\nclass B(val call: Int)\ntypealias Handler = suspend B.() -> Unit\nfun post(body: Handler) {}\n\
         //- Main.kt\npackage app\nimport routing.post\nfun setup() { post { /*^*/call } }\n",
    );
}

#[test]
fn implicit_receiver_extension_resolves_through_generic_function_type_alias() {
    check(
        "//- Pipeline.kt\npackage pipeline\nclass ApplicationCall\nclass PipelineContext<T, C>\nclass Other\nval <C : ApplicationCall> PipelineContext<Unit, C>./*def*/call: C get() = TODO()\nval Other.call: Int get() = 1\ntypealias Handler<C> = suspend PipelineContext<Unit, C>.(Unit) -> Unit\nfun handle(body: Handler<ApplicationCall>) {}\n\
         //- Main.kt\npackage app\nimport pipeline.handle\nimport pipeline.call\nfun setup() { handle { /*^*/call } }\n",
    );
}
