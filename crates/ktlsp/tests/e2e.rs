...
async fn initialize_open_and_goto_definition() {
    ...
    assert!(
        init.capabilities.hover_provider.is_some(),
    ...
    assert!(
        ...
        "server must advertise rename support"
    );
    assert!(
    ...
    assert!(
        ...
        "server must advertise type hierarchy support"
    );
    assert!(
    ...
    assert!(
        ...
        "server uses pushed diagnostics; advertising pull diagnostics makes clients keep duplicate namespaces"
    );
    ...
    match workspace_diag {
        WorkspaceDiagnosticReportResult::Report(report) => {
            assert_eq!(
                report.items.len(),
                1,
                "one open buffer should be reported: {report:?}"
            );
        }
...
async fn completion_auto_import_edit() {
    ...
    assert!(
        edits[0].new_text.starts_with("import "),
        "edit must insert an import: {edits:?}"
    );
...
async fn expanded_editor_surface_requests_round_trip() {
    ...
    let greeter_line = text
        .lines()
        .position(|line| line.contains("interface Greeter"))
        .unwrap() as u32;
    let greeter_col = text
        .lines()
        .nth(greeter_line as usize)
        .unwrap()
        .find("Greeter")
        .unwrap() as u32;
    ...
    let g_line = text
        .lines()
        .position(|line| line.contains("println(g)"))
        .unwrap() as u32;
    let g_col = text
        .lines()
        .nth(g_line as usize)
    ...
    let call_line = text
        .lines()
        .position(|line| line.contains("add(1, 2)"))
        .unwrap() as u32;
    let call_col = text
        .lines()
    ...
    assert!(
        sig.signatures.iter().any(|s| s.label.contains("add(")),
        "{sig:?}"
    );
    ...
    assert!(
        subtypes.iter().any(|item| item.name == "ConsoleGreeter"),
        "{subtypes:?}"
    );
    ...
    let missing_line = text
        .lines()
        .position(|line| line.contains("MissingHarnessType"))
        .unwrap() as u32;
    ...
    assert!(
        formatting.is_none(),
        "cat formatter should produce no edits"
    );
