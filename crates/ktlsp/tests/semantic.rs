...
#[test]
fn java_semantic_tokens_classify_declarations_and_obvious_usages() {
    let mut ws = Workspace::new();
    let key = "mem:///Main.java".to_string();
    let src = "package app.demo;\n\
               interface Worker { void work(String name); }\n\
               class Helper implements Worker {\n\
               \x20\x20\x20\x20private String label;\n\
               \x20\x20\x20\x20public void work(String name) {\n\
               \x20\x20\x20\x20\x20\x20\x20\x20String local = \"x\";\n\
               \x20\x20\x20\x20\x20\x20\x20\x20this.label = name + local + 1;\n\
               \x20\x20\x20\x20}\n\
               }\n\
               // trailing\n";
    ws.open(key.clone(), src.to_string());
    ...
    assert_token(&tokens, src, "app", SemanticTokenKind::Namespace);
    assert_token(&tokens, src, "demo", SemanticTokenKind::Namespace);
    assert_token(&tokens, src, "Worker", SemanticTokenKind::Interface);
    assert_token(&tokens, src, "Helper", SemanticTokenKind::Class);
    assert_token(&tokens, src, "work", SemanticTokenKind::Function);
    assert_token(&tokens, src, "name", SemanticTokenKind::Parameter);
    assert_token(&tokens, src, "label", SemanticTokenKind::Property);
    assert_token(&tokens, src, "local", SemanticTokenKind::Variable);
    assert_token(&tokens, src, "\"x\"", SemanticTokenKind::String);
    assert_token(&tokens, src, "1", SemanticTokenKind::Number);
    assert_token(&tokens, src, "// trailing", SemanticTokenKind::Comment);
}

#[test]
fn java_semantic_tokens_mark_method_declaration_and_usage() {
    let mut ws = Workspace::new();
    let key = "mem:///Main.java".to_string();
    ...
    let tokens = ws.semantic_tokens(&key);
    let work_tokens = tokens
        .iter()
        .filter(|token| &src[token.start_byte..token.end_byte] == "work")
        .collect::<Vec<_>>();

    assert!(
        work_tokens.iter().any(|token| token.declaration),
        "method declaration should be marked: {work_tokens:?}"
    );
    assert!(
        work_tokens.iter().any(|token| !token.declaration),
        "method usage should not be marked: {work_tokens:?}"
    );
}
