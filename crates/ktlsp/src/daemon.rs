...
impl DaemonCompiler {
    ...
    pub fn compile(
        ...
        let module = match changed {
            Some(c) => {
                let m = module_path_for(root, c).ok_or_else(|| {
                    anyhow::anyhow!("can't derive gradle module for {}", c.display())
                })?;
    ...
    fn compile_with_retry(
        &mut self,
        req: &CompileRequest,
    ) -> anyhow::Result<sidecar::CompileResult> {
...
mod tests {
    ...
    fn module_path_outside_root_is_none() {
        assert_eq!(
            module_path_for(Path::new("/repo"), Path::new("/other/A.kt")),
            None
        );
    }
}
