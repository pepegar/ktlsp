...
//!
//! Destination: `KTLSP_TRACE` when set. Writing is best-effort and never disturbs request handling.
...
/// Record one internal operation as a trace span. This is intentionally generic so hot resolution
/// paths can expose substeps without inventing a new JSON shape for each feature.
pub fn span(name: &str, category: &str, start: Instant, args: serde_json::Value) {
...
pub fn ident_at(text: &str, offset: usize) -> Option<String> {
    ...
    while start > 0
        && is_ident(bytes[start - 1])
        && (start >= bytes.len() || !is_ident(bytes[start]))
    {
        start -= 1;
    }
