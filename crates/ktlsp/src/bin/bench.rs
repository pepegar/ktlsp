...
impl KotlinDaemonBackend {
    fn new() -> Self {
        KotlinDaemonBackend {
            inner: Mutex::new(DaemonCompiler::new()),
        }
    }
}
...
fn backend_by_name(name: &str) -> anyhow::Result<Box<dyn CompileBackend>> {
    match name {
        ...
        other => anyhow::bail!("unknown backend '{other}' (known: gradle-cli, kotlin-daemon)"),
    }
}
    ...
    fn create(dir: &Path) -> anyhow::Result<Probe> {
        ...
        let original = if path.exists() {
            Some(fs::read_to_string(&path).map_err(|e| {
                anyhow::anyhow!(
                    "probe path {} exists but is unreadable: {e}",
                    path.display()
                )
            })?)
        } else {
...
fn default_probe_dir(root: &Path) -> anyhow::Result<PathBuf> {
    find_source_roots(root).pop().ok_or_else(|| {
        anyhow::anyhow!(
            "no src/main/kotlin directory found under {}",
            root.display()
        )
    })
}
...
fn diff_diags(baseline: &[NormDiag], candidate: &[NormDiag]) -> DiffResult {
    ...
    for m in missing {
        if let Some(pos) = extra
            .iter()
            .position(|e| e.severity == m.severity && e.message == m.message)
        {
...
fn cmd_oracle(args: &Args) -> anyhow::Result<ExitCode> {
    ...
    let base: Vec<NormDiag> = base_out
        .diagnostics
        .iter()
        .map(|d| normalize(d, &root))
        .collect();
    let cand: Vec<NormDiag> = cand_out
        .diagnostics
        .iter()
        .map(|d| normalize(d, &root))
        .collect();
...
fn print_oracle(r: &OracleReport) {
    ...
    println!(
        "baseline:  {} ({} diags)",
        r.baseline_backend, r.baseline_count
    );
    println!(
        "candidate: {} ({} diags)",
        r.candidate_backend, r.candidate_count
    );
    ...
    println!(
        "result:    {}",
        if r.ok { "OK (parity)" } else { "DIVERGENCE" }
    );
    println!("======================================");
}
...
fn cmd_analyze(args: &Args) -> anyhow::Result<ExitCode> {
    let path = match args.get("file") {
        ...
        None => telemetry::log_path().ok_or_else(|| {
            anyhow::anyhow!("no --file and no telemetry path (set HOME or KTLSP_COMPILE_LOG)")
        })?,
    };
...
fn print_analyze(r: &AnalyzeReport, path: &Path) {
    ...
    println!(
        "records:     {} ({} unparseable lines skipped)",
        r.total, r.skipped_lines
    );
    println!("up-to-date:  {} (no recompile)", r.up_to_date);
    println!(
        "superseded:  {} (newer save arrived mid-compile)",
        r.superseded
    );
    println!("cold:        {}{}", r.cold, cold_note(&r.cold_ms));
    println!(
        "steady-state latency over {} real compiles:",
        r.steady_count
    );
    println!(
        "  p50 {:.0} ms   p95 {:.0} ms   (min {:.0} / max {:.0})",
        r.p50_ms, r.p95_ms, r.min_ms, r.max_ms
    );
    println!("===================================================");
}
...
fn cmd_trace(args: &Args) -> anyhow::Result<ExitCode> {
    let path = match args.get("file") {
        Some(f) => PathBuf::from(f),
        None => trace::log_path().ok_or_else(|| {
            anyhow::anyhow!("no --file and no trace path (set HOME or KTLSP_TRACE)")
        })?,
    };
    let out = args
        .get("out")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("ktlsp-trace.json"));
    let content = fs::read_to_string(&path)
        .map_err(|e| anyhow::anyhow!("reading {}: {e}", path.display()))?;
...
impl Args {
    ...
    fn root(&self) -> anyhow::Result<PathBuf> {
        let r = self
            .get("root")
            .ok_or_else(|| anyhow::anyhow!("--root <dir> is required"))?;
...
mod tests {
    ...
    fn find_source_roots_detects_and_skips_noise() {
        ...
        assert_eq!(
            roots.len(),
            1,
            "exactly one real source root, build/ decoy skipped"
        );
    ...
    fn probe_removes_file_when_absent_before() {
        ...
        assert!(
            !path.exists(),
            "probe must be removed on drop when it did not exist before"
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn has_probe_error_filters_severity_and_path() {
        let err_on_probe = CompileOutcome {
            diagnostics: vec![diag(PROBE_FILE, 1, 1, Severity::Error, "x")],
            executed: true,
        };
        assert!(has_probe_error(&err_on_probe, PROBE_FILE));

        let warning = CompileOutcome {
            diagnostics: vec![diag(PROBE_FILE, 1, 1, Severity::Warning, "x")],
            executed: true,
        };
        assert!(
            !has_probe_error(&warning, PROBE_FILE),
            "a warning is not a probe error"
        );

        let other_file = CompileOutcome {
            diagnostics: vec![diag("Other.kt", 1, 1, Severity::Error, "x")],
            executed: true,
        };
        assert!(
            !has_probe_error(&other_file, PROBE_FILE),
            "error on a different file is not a probe error"
        );
    }
    ...
    fn inject_and_recover_loops_with_fake_backend_leave_tree_clean() {
        ...
        };
        {
            ...
            assert_eq!(
                inj.failures, 0,
                "fake backend reports the injected error every time"
            );
            let rec = run_recover(&backend, &dir, &probe, 3).unwrap();
            assert_eq!(rec.count, 3);
            assert_eq!(
                rec.failures, 0,
                "fake backend clears the error after the fix every time"
            );
        }
        assert!(
            !dir.join(PROBE_FILE).exists(),
            "tree must be clean after the run"
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn inject_counts_failures_when_backend_never_reports_error() {
        let dir = tmp("inject-fail");
        {
            ...
            assert_eq!(
                inj.failures, 4,
                "an always-clean backend fails every inject iteration"
            );
        }
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn nd(path: &str, line: u32, col: u32, msg: &str) -> NormDiag {
        normalize(
            &diag(path, line, col, Severity::Error, msg),
            Path::new("/root"),
        )
    }
    ...
    fn oracle_flags_wrong_location_as_mislocated() {
        ...
        assert_eq!(
            d.mislocated.len(),
            1,
            "same message at a different location is mislocated, not match"
        );
    ...
    fn analyze_steady_state_excludes_cold_uptodate_superseded() {
        ...
        assert_eq!(
            r.steady_count, 3,
            "only the 3 executed/published/warm compiles"
        );
