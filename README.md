...
- `workspace/executeCommand` commands: `ktlsp.reindex`, `ktlsp.tracePath`,
  `ktlsp.explainResolution`, `ktlsp.explainCompletion`, and `ktlsp.dumpSymbol`.
...
- `artifacts/trace-events.jsonl` — per-request ktlsp events from `KTLSP_TRACE`, including `cat:"lsp"`
  timing events and `cat:"semantic"` explainability events for hover/completion.
