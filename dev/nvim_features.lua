...
do
  local l, c = find("showGreeter(helper())", "helper")
...
do
  ...
  local ml, mc = find("fun renderFeature()", "renderFeature")
  check("found FeatureSurface member", ml ~= nil)
  if ml then
    local impls = request("textDocument/implementation", {
      textDocument = { uri = uri },
      position = { line = ml, character = mc },
    })
    local loc = impls and (impls[1] or impls)
    check("implementation(renderFeature) returns overriding member", loc ~= nil and loc.range ~= nil, vim.inspect(impls))
  end

  local gl, gc = find("showGreeter(g.greet())", "g")
...
do
  -- Position the cursor one char into the `helper` token of the `showGreeter(helper())` call, so the
...
do
  local l, c = find("        greet()", "greet")
  check("found bare greet() inside receiver lambda", l ~= nil)
  if l then
    local res = request("textDocument/definition", { textDocument = { uri = uri }, position = { line = l, character = c } })
    local loc = res
    if type(res) == "table" and res[1] then
    ...
    check("implicit receiver goto greet() -> Greeter.kt", loc ~= nil and loc.uri ~= nil and loc.uri:match("Greeter%.kt$") ~= nil, loc and loc.uri)
  end
end

do
  local l, c = find("        setTag()", "setTag")
  check("found bare setTag() inside custom receiver lambda", l ~= nil)
  if l then
    local res = request("textDocument/definition", { textDocument = { uri = uri }, position = { line = l, character = c } })
    local loc = res
    if type(res) == "table" and res[1] then
    ...
    check("implicit receiver goto setTag() -> Main.kt", loc ~= nil and loc.uri ~= nil and loc.uri:match("Main%.kt$") ~= nil, loc and loc.uri)
  end
end
